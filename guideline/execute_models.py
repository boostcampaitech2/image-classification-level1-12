...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="train",
        help="set mode to execute models (default : train)",
    )
    parser.add_argument(
        "--lr", type=float, default=0.0001, help="set learning rate (default : 0.0001)"
    )
    parser.add_argument(
        "--seed", type=int, default=12, help="set random seed number (default : 12)"
    )
    parser.add_argument(
        "--epochs", type=int, default=5, help="set epochs (default :5 , max : 10)"
    )
    ...
    if args.mode == "train":
        ...
        data_loader = DataLoader(
            datasets, batch_size=128, shuffle=True
        )  # , num_workers=2)
        ...
        lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer=optm, mode="max", factor=0.5, patience=2
        )
        ...
        for epoch in range(args.epochs):
            ...
            for i, (images, labels) in enumerate(tqdm.tqdm(data_loader, leave=False)):
                ...
                epoch_f1 += f1_score(
                    labels.cpu().numpy(), predict.cpu().numpy(), average="macro"
                )
            ...
            if epoch_f1 / (epoch + 1) > 90 and loss_value_sum / len(data_loader) < 0.7:
                torch.save(
                    model.state_dict(),
                    "./pre_model_results/" + now + f"_{epoch}_stopped.pt",
                )
